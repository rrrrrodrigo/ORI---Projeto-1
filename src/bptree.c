...
static inline const uint8_t *val_slot_c(const uint8_t *page, int i,
                                         size_t ksz, uint16_t ord)
{
    return page + 8u + (size_t)ord * ksz + (size_t)i * VAL_SLOT_SIZE;
}

/* next_leaf pointer: 8 bytes immediately after all value slots. */
static inline uint64_t leaf_next_get(const uint8_t *page, size_t ksz,
                                      uint16_t ord)
{
    return rd64(page + 8u + (size_t)ord * ksz + (size_t)ord * VAL_SLOT_SIZE);
}

static inline void leaf_next_set(uint8_t *page, size_t ksz, uint16_t ord,
                                  uint64_t pg)
{
 ...
 */
static inline uint64_t child_get(const uint8_t *page, int i, size_t ksz,
                                   uint16_t ord)
{
    return rd64(page + 8u + (size_t)ord * ksz + (size_t)i * 8u);
}

static inline void child_set(uint8_t *page, int i, size_t ksz, uint16_t ord,
                               uint64_t pg)
{
...
static inline uint8_t  node_type(const uint8_t *page) { return page[0]; }
static inline uint16_t node_nkeys(const uint8_t *page) { return rd16(page + 1); }
static inline void     node_set_type(uint8_t *page, uint8_t t) { page[0] = t; }
static inline void     node_set_nkeys(uint8_t *page, uint16_t n) { wr16(page + 1, n); }
 ...
 */
static int lower_bound(const uint8_t *page, uint16_t nk, size_t ksz,
                        bptree_cmp_fn cmp, const void *key)
{
    int lo = 0, hi = (int)nk;
    while (lo < hi) {
        int mid = lo + (hi - lo) / 2;
 ...
 */
static int upper_bound(const uint8_t *page, uint16_t nk, size_t ksz,
                        bptree_cmp_fn cmp, const void *key)
{
    int lo = 0, hi = (int)nk;
    while (lo < hi) {
...
int bptree_cmp_training(const void *a, const void *b)
{
    ...
    if (ta.id_usuario  != tb.id_usuario)  return (ta.id_usuario  < tb.id_usuario)  ? -1 : 1;
    if (ta.id_exercicio != tb.id_exercicio) return (ta.id_exercicio < tb.id_exercicio) ? -1 : 1;
    if (ta.data        != tb.data)        return (ta.data        < tb.data)        ? -1 : 1;
    if (ta.offset      != tb.offset)      return (ta.offset      < tb.offset)      ? -1 : 1;
    return 0;
}
...
{
    if (path == NULL || key_size == 0 || key_size > BPTREE_MAX_KEY_SIZE
            || cmp == NULL) {
        errno = EINVAL;
        return NULL;
    }
...
bool bptree_search(bptree_t *bt, const void *key, bptree_val_t *out_val)
{
    ...
    for (;;) {
        ...
        if (node_type(buf) == NODE_INTERNAL) {
            ...
            if (pos >= (int)nk
                    || bt->cmp(key_ptr_c(buf, pos, bt->key_size), key) != 0) {
                errno = ENOENT;
                return false;
            }
 ...
 */
static void leaf_copy_entry(uint8_t *dst, int dst_i,
                             const uint8_t *src, int src_i,
                             size_t ksz, uint16_t ord)
{
    memcpy(key_ptr(dst, dst_i, ksz), key_ptr_c(src, src_i, ksz), ksz);
    memcpy(val_slot(dst, dst_i, ksz, ord),
           val_slot_c(src, src_i, ksz, ord), VAL_SLOT_SIZE);
}
 ...
 */
static void leaf_write_entry(uint8_t *dst, int dst_i,
                              const void *key, bptree_val_t val,
                              size_t ksz, uint16_t ord)
{
 ...
 */
static bool split_leaf(bptree_t *bt,
                        uint8_t *left_buf, uint64_t left_pg,
                        uint8_t *right_buf, uint64_t *right_pg,
                        int pos, const void *key, bptree_val_t val,
                        uint8_t *sep_key)
{
 ...
 */
static bool split_internal(bptree_t *bt,
                            uint8_t *left_buf, uint64_t left_pg,
                            uint8_t *right_buf, uint64_t *right_pg,
                            int pos,
                            const uint8_t *sep_in, uint64_t right_child_in,
                            uint8_t *sep_out)
{
    size_t   ksz = bt->key_size;
    uint16_t ord = bt->order;
    ...
    if (pos == mid) {
        ...
        for (int j = 0; j < (int)ord - mid; j++) {
            ...
            child_set(right_buf, j + 1, ksz, ord,
                      child_get(left_buf, mid + j + 1, ksz, ord));
        }
    } else if (pos < mid) {
        ...
        child_set(right_buf, 0, ksz, ord,
                  child_get(left_buf, mid, ksz, ord));
        for (int j = 0; j < (int)ord - mid; j++) {
            memcpy(key_ptr(right_buf, j, ksz), key_ptr(left_buf, mid + j, ksz), ksz);
            child_set(right_buf, j + 1, ksz, ord,
                      child_get(left_buf, mid + j + 1, ksz, ord));
        ...
        for (int i = mid - 1; i > pos; i--) {
            ...
            child_set(left_buf, i + 1, ksz, ord,
                      child_get(left_buf, i, ksz, ord));
        }
        ...
        child_set(right_buf, 0, ksz, ord,
                  child_get(left_buf, mid + 1, ksz, ord));
        for (int j = 0; j < adj; j++) {
            memcpy(key_ptr(right_buf, j, ksz), key_ptr(left_buf, mid + 1 + j, ksz), ksz);
            child_set(right_buf, j + 1, ksz, ord,
                      child_get(left_buf, mid + 2 + j, ksz, ord));
        }
        ...
        for (int j = adj + 1; j <= right_keys; j++) {
            memcpy(key_ptr(right_buf, j, ksz),
                   key_ptr(left_buf, mid + j, ksz), ksz);
            child_set(right_buf, j + 1, ksz, ord,
                      child_get(left_buf, mid + 1 + j, ksz, ord));
        }
    }
...
bool bptree_insert(bptree_t *bt, const void *key, bptree_val_t val)
{
    ...
    if (!split_leaf(bt, buf, leaf_pg, right_buf, &right_pg,
                    pos, key, val, sep)) {
        return false;
    }
    bt->num_keys++;
    ...
    while (depth > 0) {
        ...
        if (par_nk < bt->order) {
            ...
            for (int i = (int)par_nk; i > par_slot; i--) {
                memcpy(key_ptr(par_buf, i, bt->key_size),
                       key_ptr(par_buf, i - 1, bt->key_size), bt->key_size);
                child_set(par_buf, i + 1, bt->key_size, bt->order,
                          child_get(par_buf, i, bt->key_size, bt->order));
            }
        ...
        if (!split_internal(bt, par_buf, par_pg,
                            par_right_buf, &par_right_pg,
                            par_slot, sep, new_right, new_sep)) {
            return false;
        }
...
typedef struct {
    ...
    uint8_t  lo[BPTREE_MAX_KEY_SIZE]; bool has_lo;
    uint8_t  hi[BPTREE_MAX_KEY_SIZE]; bool has_hi;
} VerifyFrame;
...
bool bptree_verify(const bptree_t *bt)
{
    ...
    while (dfs_top > 0) {
        ...
        for (int i = 1; i < (int)nk; i++) {
            if (bt->cmp(key_ptr_c(buf, i - 1, bt->key_size),
                        key_ptr_c(buf, i,     bt->key_size)) >= 0) {
        ...
        /* Bounds from parent. */
        if (f.has_lo && nk > 0
                && bt->cmp(key_ptr_c(buf, 0, bt->key_size), f.lo) < 0) {
            printf("verify: page %" PRIu64 " first key below parent lower bound\n",
                   f.pg);
            return false;
        }
        if (f.has_hi && nk > 0
                && bt->cmp(key_ptr_c(buf, (int)nk - 1, bt->key_size), f.hi) > 0) {
        ...
        if (type == NODE_LEAF) {
            ...
            for (int i = 0; i < (int)nk; i++) {
                if (val_slot_c(buf, i, bt->key_size, bt->order)[0]
                        == TOMBSTONE_ACTIVE) {
                    active_count++;
                }
            }
            ...
            for (int i = (int)nk; i >= 0; i--) {
                ...
                if (i > 0) {
                    cf.has_lo = true;
                    memcpy(cf.lo, key_ptr_c(buf, i - 1, bt->key_size),
                           bt->key_size);
                } else {
                    cf.has_lo = f.has_lo;
                    if (f.has_lo) {
    ...
    if (active_count != bt->num_keys) {
        printf("verify: header num_keys=%" PRIu64 " but counted %" PRIu64
    ...
    while (chain_pg != 0) {
        ...
        if (chain_pg != leaf_pages_dfs[chain_i]) {
            printf("verify: leaf chain page %" PRIu64
                   " != DFS leaf[%d]=%" PRIu64 "\n",
